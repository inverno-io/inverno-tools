...
public abstract class AbstractContainerImageMojo extends BuildApplicationMojo {
	 ...
	 */
	@Parameter(property = "winter.container.registry", required = false)
	private String registry;
	
	/**
	 * The repository part of the target image reference defined as: <code>${registry}/${repository}/${name}:${project.version}</code>
	 */
	@Parameter(property = "winter.container.repository", required = false)
	private String repository;
	
	/**
	 * The format of the container image.
	 */
	@Parameter(property = "winter.container.imageFormat", defaultValue = "Docker", required = false)
	...
	protected void doExecute() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			this.projectModule = new ProjectModule(this.project, projectModuleReference.descriptor(), dependencies, this.winterBuildPath, this.jmodsPath, ProjectModule.Classifier.CONTAINER, Set.of());
