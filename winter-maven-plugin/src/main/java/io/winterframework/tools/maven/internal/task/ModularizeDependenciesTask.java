...
public class ModularizeDependenciesTask extends Task<Set<DependencyModule>> {
		...
		try {
			...
			for(DependencyModule dependency : this.projectModule.getModuleDependencies()) {
				if(dependency.isMarked()) {
					if(dependency.getModuleDescriptor().isAutomatic()) {
						if(this.verbose) {
			...
			throw new TaskExecutionException("Error modularizing dependencies, activate '-Dwinter.image.verbose=true' to display full log", e);
		}
	}
	...
	private void unpackDependency(DependencyModule dependency) throws TaskExecutionException {
		...
		try(JarFile moduleJar = new JarFile(dependency.getSourcePath().toFile(), true, ZipFile.OPEN_READ, Runtime.version())) {
			...
			throw new TaskExecutionException("Error unpacking dependency " + dependency + ", activate '-Dwinter.image.verbose=true' to display full log", e);
		}
	}
	...
	private void generateModuleInfo(DependencyModule dependency) throws TaskExecutionException {
		try {
			...
			if(this.jdeps.run(this.verbose ? this.getOutStream() : new NullPrintStream(), this.getErrStream(), jdeps_args) == 0) {
				...
				throw new TaskExecutionException("Error generating module-info.java for " + dependency + ", activate '-Dwinter.image.verbose=true' to display full log");
			}
		}
		catch (IOException e) {
			throw new TaskExecutionException("Error generating module-info.java for " + dependency + ", activate '-Dwinter.image.verbose=true' to display full log", e);
		}
	}
}
