...
public final class Service2GrpcClient {
	...
	public Service2GrpcClient(HttpClient httpClient, GrpcClient grpcClient) {
		this.httpClient = httpClient;
		this.grpcClient = grpcClient;
	}

	public <A extends ExchangeContext> Service2GrpcClient.Stub<A> createStub(String host, int port) {
		return new Service2GrpcClient.StubImpl<>(this.httpClient.<A>endpoint(host, port).build(), true);
	}
	
	public <A extends ExchangeContext> Service2GrpcClient.Stub<A> createStub(InetSocketAddress remoteAddress) {
		return new Service2GrpcClient.StubImpl<>(this.httpClient.<A>endpoint(remoteAddress).build(), true);
	}
	
	public <A extends ExchangeContext> Service2GrpcClient.Stub<A> createStub(Endpoint<A> endpoint) {
		return new Service2GrpcClient.StubImpl<>(endpoint, false);
	}
	
	private final class StubImpl<A extends ExchangeContext> implements Service2GrpcClient.Stub<A> {
		...
		private StubImpl(Service2GrpcClient.StubImpl<A> parent, Consumer<GrpcOutboundRequestMetadata> metadataConfigurer) {
		...
		@Override
		public Service2GrpcClient.Stub<A> withMetadata(Consumer<GrpcOutboundRequestMetadata> metadataConfigurer) {
			return new Service2GrpcClient.StubImpl<>(this, metadataConfigurer);
		}
		...
		public Mono<GrpcExchange.ClientStreaming<A, test.multi.Svc2Request, test.multi.Svc2Response>> call(A context) {
			return this.endpoint.exchange(context)
				.map(exchange -> {
					GrpcExchange.ClientStreaming<A, test.multi.Svc2Request, test.multi.Svc2Response> grpcExchange = Service2GrpcClient.this.grpcClient.clientStreaming(exchange, SERVICE_NAME, "call", test.multi.Svc2Request.getDefaultInstance(), test.multi.Svc2Response.getDefaultInstance());
	 ...
	 */
	public interface Stub<A extends ExchangeContext> extends GrpcClient.Stub<A, Service2GrpcClient.Stub<A>> {
